...
PYTHONUNBUFFERED=1 uv run --extra benchmark python scripts/run_allinstr_showcase.py \
  ...
  --persona-library-scenarios data/persona_library_selections/pure_authority_qwen3_14b_strict25.jsonl \
  --n-pairs 25 \
  ...
  --admin-top-p 0.95 \
  --c-grid 0.5,1 \
  --out outputs/20260630_pure_authority_qwen3_14b_strict25_sspace_mfv_mfq2_n8 \
  "$@"
