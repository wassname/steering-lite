...
from steering_lite.data import (
    PERSONA_PAIRS_TRAD_CARE, PROMPT_TEMPLATE, load_suffixes,
)
...
def _build_repeng_dataset(tok, n_pairs: int, seed: int = 42):
    ...
    pos_personas = [p for p, _ in PERSONA_PAIRS_TRAD_CARE]
    neg_personas = [n for _, n in PERSONA_PAIRS_TRAD_CARE]
...
def main() -> None:
    ...
    ap.add_argument("--coeff", type=float, default=1.5,
                    help="raw control coefficient (uncalibrated, vgel default)")
    ...
    cmodel.set_control(control, coefficient=args.coeff)
    t0 = time.time()
